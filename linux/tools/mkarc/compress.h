...
struct LzmaCtx {
    ...
    void destroy() {
        ...
        if (enc) {
            LzmaEnc_Destroy(enc, &g_Alloc4x4, &g_Alloc4x4);
            enc = nullptr;
        }
        initialized = false;
    }
        ...
        std::thread writerThread([&]() {
            ...
            while (!writerDone || !pending.empty()) {
                ...
                std::sort(local.begin(), local.end(),
                    [](Job4x4 *a, Job4x4 *b) { return a->seq < b->seq; });
